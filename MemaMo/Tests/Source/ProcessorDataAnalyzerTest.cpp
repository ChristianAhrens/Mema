...
//
// NOTE [bug]: both the level and spectrum computations in analyzeData() read
// their input from m_centiSecondBuffer -- but that buffer is only ever written
// to (via copyFrom()) inside the `if (isBufferProcessingUsed())` branch. If
// buffer processing is NOT enabled, level/spectrum silently compute from
// whatever m_centiSecondBuffer happened to contain before (stale/uninitialised
// memory, not the actual signal just passed in) rather than from the real
// input. This is not just a theoretical corner case: MemaProcessor.cpp calls
// `setUseProcessingTypes(true, false, false)` for its own input/output
// analyzers (level only, no buffer) -- so Mema's own on-device level metering
// currently reads garbage, not real levels. MemaMo never hits this because it
// always pairs level/spectrum with buffer processing (see MemaMoComponent.cpp).
// The tests below deliberately use MemaMo's real (working) call pattern --
// buffer processing enabled alongside level/spectrum -- both to get
// deterministic, correct results and to avoid asserting on undefined behaviour;
// see the accompanying report for the suggested fix to analyzeData() itself.
//
...
public:
    ...
    void runTest() override
    {
        ...
        beginTest ("A constant-amplitude buffer produces the exact expected peak/RMS level");
        {
            ...
            analyzer.setUseProcessingTypes (true, true, false); // level + buffer, matching MemaMoComponent's real usage
            expect (analyzer.IsInitialized());
...
public:
    ...
    void runTest() override
    {
        ...
        ProcessorDataAnalyzer analyzer;
        analyzer.initializeParameters (48000.0, 480);
        analyzer.setUseProcessingTypes (true, true, true); // level + buffer + spectrum, matching MemaMoComponent's real usage
